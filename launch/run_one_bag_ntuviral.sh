...
roslaunch sr_lio lio_viral.launch autorun:=true \
...
-loglevel quiet -t $LOG_DUR -y /media/neo/niobe/datasets/NTUViral/benchmarking/$EXP_NAME.avi \
& \
else
...
& \
timeout $LOG_DUR rostopic echo -p --nostr --noarr /Odometry_after_opt > $EXP_OUTPUT_DIR/predict_odom.csv \
& \
